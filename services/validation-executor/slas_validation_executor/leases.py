"""Exclusive target leases (CLAUDE.md §10.2 guardrails): one run per target at a time."""
        ...
        if current is not None and current.until > now and current.ticket_id != ticket_id:
            raise LeaseError(
                ThreePartMessage(
                    ...
                    "Only one run may use a target at a time (exclusive lease).",
                    "Pick another target, or wait for that run to finish or its lease to expire.",
                )
            )
        lease = Lease(
            target=target,
