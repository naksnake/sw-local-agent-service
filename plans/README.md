...
| `schema/factory-plan.schema.json`, `primitives/factory.yaml` | The Factory verbs (P9), rendered from `slas_factory_executor.primitives`: `lease_station`, `station_command`, `skill`, `wait_for_screen`, `read_result`, `read_sensors`, `check_event_log`, `verdict`, `backup_station`, `release_station`; `station_config_change` is destructive. Every verb acts on a station through the station runner. |

Compiled plans land in `${SLAS_DATA_ROOT}/Validation/Plans/<plan-id>/plan.yaml` and
`${SLAS_DATA_ROOT}/Factory/Plans/<plan-id>/plan.yaml`.
