...
| R4 | 以 `nvidia-smi` 詢問主機有哪些 GPU，在 `SLAS_GPU_IDS` 與 `SLAS_GPU_VRAM_GIB` 仍為範本猜測值時寫入 `.env`（最小的卡減去 6 % 餘裕）；寫入其餘 `.env` 與密鑰檔；以你的使用者身分建立各服務綁定掛載的資料目錄：`Coding`、`Toolchains`、`.git-broker`、`Tickets`、`Skills/library`、`SOP`、`Validation`、`Factory/{Templates,mes/inbox,ca}`、`Models`、`Knowledge`、`Backups/stations`、`qdrant`、`tls` | 「2 GPUs found (80 GiB each): .env gets SLAS_GPU_IDS=0,1 and SLAS_GPU_VRAM_GIB=74 …」、「Created N data directories under /AI/Agent as uid …」 |
| R5 | 放置權重、`docker compose up -d --pull never`、等待健康檢查、印出登入網址 | 「SW Local Agent Service is up.」 |
| R6 | 最多等待 15 分鐘（`SLAS_MODEL_WAIT_S`）讓模型實例載入，並印出每個實例的狀態與句子；當 coder 角色沒有健康的實例時以 exit 1 結束並說明原因（當機與其最後幾行日誌、模型放不進 GPU、沒有 GPU） | 「vllm-coder (coder): healthy — … The coding model is ready; start a coding task from the Coding page.」 |
