...
| R4 | asks `nvidia-smi` which GPUs the host has and writes `SLAS_GPU_IDS` and `SLAS_GPU_VRAM_GIB` (the smallest card minus 6 % headroom) into `.env` while they are at the template's guesses; writes the rest of `.env` and the secret files; creates the data directories the services bind-mount, as your user: `Coding`, `Toolchains`, `.git-broker`, `Tickets`, `Skills/library`, `SOP`, `Validation`, `Factory/{Templates,mes/inbox,ca}`, `Models`, `Knowledge`, `Backups/stations`, `qdrant`, `tls` | "2 GPUs found (80 GiB each): .env gets SLAS_GPU_IDS=0,1 and SLAS_GPU_VRAM_GIB=74 …", "Created N data directories under /AI/Agent as uid …" |
...
| R6 | waits up to 15 minutes (`SLAS_MODEL_WAIT_S`) for the model instances and prints each one's state and sentence; ends with exit 1 and the reason when the coder role has no healthy instance (a crash with its last log lines, a model that does not fit the GPUs, no GPU) | "vllm-coder (coder): healthy — … The coding model is ready; start a coding task from the Coding page." |
...
| Preflight ✗ GPU fabric, or every model instance "keeps crashing" with `Error 802: system not yet initialized` | an NVSwitch system (SXM GPUs such as B300, B200, H100 SXM) without NVIDIA Fabric Manager: nvidia-smi works, CUDA does not | `sudo apt install nvidia-fabricmanager-<driver major>` (595 for driver 595.x), `sudo systemctl enable --now nvidia-fabricmanager`, wait for `nvidia-smi --query-gpu=fabric.state --format=csv` to show Completed, then `./install.sh` again |
