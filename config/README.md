...
|---|---|---|
| `.env.example` | P0 | Template `install.sh` copies to `${SLAS_DATA_ROOT}/.env`; one sentence per key. `SLAS_RUNTIME_SOCKET` (empty = Podman's socket; the installer writes Docker's when only that one exists, ADR-0015) and `SLAS_GPU_VRAM_GIB` (per-GPU budget the model manager places instances against, default 180) are the round-2 host knobs. |
